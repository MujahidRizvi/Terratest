...
type TestCaseMod struct {
	XMLName   xml.Name    `xml:"testcase"`
	Classname string      `xml:"classname,attr"`
	Name      string      `xml:"name,attr"`
	Failure   *FailureMod `xml:"failure,omitempty"`
	Status    string      `xml:"status"`
}

type FailureMod struct {
	Message string `xml:"message,attr"`
	Type    string `xml:"type,attr"`
}

type TestSuiteMod struct {
	XMLName   xml.Name      `xml:"testsuite"`
	Tests     int           `xml:"tests,attr"`
	Failures  int           `xml:"failures,attr"`
	Errors    int           `xml:"errors,attr"`
	Time      float64       `xml:"time,attr"`
	TestCases []TestCaseMod `xml:"testcase"`
}

func loadTFStateMod(t *testing.T, path string) map[string]interface{} {
...
func mergeResourcesMod(tfState1, tfState2 map[string]interface{}) map[string]interface{} {
...
func findResourcesByTypeMod(tfState map[string]interface{}, resourceType string) []map[string]interface{} {
...
func TestAzureInfraValidationsMod(t *testing.T) {
	var suite TestSuiteMod
	suite.Tests = 9

	tfState1 := loadTFStateMod(t, "../terraform.tfstate")
	tfState2 := loadTFStateMod(t, "../terra.tfstate")
	merged := mergeResourcesMod(tfState1, tfState2)
	...
	tests := []struct {
		...
		{"1._Verify_Spoke_VNet", func() (bool, string) {
			vnets := findResourcesByTypeMod(merged, "azurerm_virtual_network")
		...
		{"2._Verify_Subnets_with_correct_CIDRs", func() (bool, string) {
			subnets := findResourcesByTypeMod(merged, "azurerm_subnet")
		...
		{"3._Verify_NSG_and_Rules", func() (bool, string) {
			nsgs := findResourcesByTypeMod(merged, "azurerm_network_security_group")
		...
		{"4._Verify_Bastion_VM_Public_IP_and_Admin_User", func() (bool, string) {
			vms := findResourcesByTypeMod(merged, "azurerm_virtual_machine")
			for _, vm := range vms {
		...
		{"5._Verify_APIM_internal_network_and_DNS", func() (bool, string) {
			apims := findResourcesByTypeMod(merged, "azurerm_api_management")
		...
		{"7._Verify_Storage_Account_Replication", func() (bool, string) {
			sas := findResourcesByTypeMod(merged, "azurerm_storage_account")
			for _, sa := range sas {
		...
		{"8._Verify_Managed_Identity", func() (bool, string) {
			ids := findResourcesByTypeMod(merged, "azurerm_managed_identity")
			for _, id := range ids {
				if id["client_id"] == nil {
		...
		{"9._Verify_App_Service_Plan_Location", func() (bool, string) {
			plans := findResourcesByTypeMod(merged, "azurerm_app_service_plan")
	...
	// Write XML report
	suite.Time = 2.15
	file, err := os.Create("reports\\bastion_report.xml")
	if err != nil {
